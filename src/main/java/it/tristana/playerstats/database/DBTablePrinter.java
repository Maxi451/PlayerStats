...
public class DBTablePrinter {
	 ...
	 */
	public static List<String> printResultSet(ResultSet rs) {
	 ...
	 * @param maxStringColWidth Max. width of text columns
	 */
	public static List<String> printResultSet(ResultSet rs, int maxStringColWidth) {
		List<String> lines = new ArrayList<String>();
		try {
			if (rs == null) {
				lines.add("DBTablePrinter Error: Result set is null!");
				return lines;
			}
			if (rs.isClosed()) {
				lines.add("DBTablePrinter Error: Result Set is closed!");
				return lines;
			}
			if (maxStringColWidth < 1) {
				lines.add("DBTablePrinter Info: Invalid max. varchar column width. Using default!");
			...
			String line = strToPrint.toString();
			...
			lines.add("SQL exception in DBTablePrinter. Message:");
